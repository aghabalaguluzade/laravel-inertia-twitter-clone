<template>
    <div v-for="comment in comments" :key="comment.id" class="flex w-full h-auto cursor-pointer hover:bg-lowWhite ">
        <div class="p-3">
            <img :src="comment.user.profile_photo_path" class="w-[45px] rounded-full" :alt="comment.user.username" />
        </div>
        <div class="w-full">
            <div class="text-sm px-2 flex  items-center justify-start gap-2 text-normalWhite mt-2">
                <h3 class="font-bold hover:underline"><Link :href="comment.user.username">{{ comment.user.name }}</Link></h3>
                <span class="text-lowsWhite font-light flex-grow"><Link :href="comment.user.username">@{{ comment.user.username }}</Link> · {{ formatDateString(comment.created_at) }}</span>
                <span>
            ...
            <ul class="flex items-center w-full justify-start gap-10 py-3">
                ...
                <li class="flex items-center gap-1 group text-sm text-lowsWhite transition duration-200 fill-lowsWhite  hover:fill-myPink hover:text-myPink cursor-pointer" :class="{ 'fill-myPink' : tweetStats.liked  }">
                    <Link preserve-scroll method="POST" as="button" :href="`/tweets/${comment.id}/like`"> 
                        <TwitLike></TwitLike>
                    </Link>   
